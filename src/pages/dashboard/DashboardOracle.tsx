import {
  ...
  CircleCheck, AlertTriangle, Wifi, ArrowDownLeft, ShieldCheck, Zap, Landmark, Smartphone, ExternalLink,
} from "lucide-react";
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <SummaryKPI icon={Wifi} label="Active Sensors" value={`${activeSensors}`} subtext="All online" />
        <SummaryKPI icon={Satellite} label="NDVI (Canopy)" value={Number(currentNdvi).toFixed(2)} subtext="Threshold: ≥ 0.55" />
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            <Card className="lg:col-span-2">
              <CardHeader>
                ...
                <CardDescription className="text-xs">Oracle threshold: ≥ 0.55 for Milestone 3 disbursement.</CardDescription>
              </CardHeader>
              <CardContent>
                ...
                ) : <p className="text-sm text-muted-foreground">No NDVI data yet.</p>}
              </CardContent>
            </Card>
            <Card>
              ...
              <CardContent className="space-y-4">
                <div className="space-y-2">
                  <div className="flex justify-between text-xs">
                    ...
                    <span className="font-bold text-primary">{Number(currentNdvi).toFixed(2)}</span>
                  </div>
                  <Progress value={(Number(currentNdvi) / 0.8) * 100} className="h-2" />
                </div>
                <div className="space-y-2">
                  <div className="flex justify-between text-xs">
                  ...
                  {Number(currentNdvi) >= 0.55 && (
                    <Badge variant="default" className="text-[10px]"><CircleCheck className="h-3 w-3 mr-1" />Threshold met</Badge>
                  )}
                </div>
