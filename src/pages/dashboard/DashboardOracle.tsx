import {
  Activity, Droplets, Thermometer, CloudRain, Satellite, Weight, Wind,
  CircleCheck, AlertTriangle, Wifi, ArrowDownLeft, ShieldCheck, Zap,
...
import { useSPVData, type OracleEvent } from "@/contexts/SPVDataContext";

const sensorIcons: Record<string, React.ElementType> = {
  "Soil Moisture": Droplets, Temperature: Thermometer, "Relative Humidity": Wind,
  "Rainfall (7-day)": CloudRain, "Cherry Intake (today)": Weight,
};
...
const ndviConfig: ChartConfig = { value: { label: "NDVI", color: "hsl(var(--primary))" } };
...
function getPaymentChannelBadge(event: OracleEvent) {
  if (!event.payment_channel) return null;
  const ch = event.payment_channel;
    ...
    <div className="flex gap-3 p-3 rounded-lg border border-border bg-card hover:bg-accent/5 transition-colors">
      ...
      <div className="flex-1 min-w-0 space-y-1.5">
        ...
        <div className="flex flex-wrap items-center gap-2">
          {event.amount && <span className="text-xs font-bold text-foreground">{event.amount.toLocaleString()} {event.currency}</span>}
...
export default function DashboardOracle() {
  const { oracleEvents, sensorReadings, ndviReadings, spv } = useSPVData();

  if (!spv) return <div className="p-8 text-muted-foreground">No SPV found.</div>;

  const totalPayments = oracleEvents.filter((e) => e.event_type === "payment_received" || e.event_type === "payment_confirmed").length;
  const ndviCurrent = ndviReadings.length > 0 ? ndviReadings[ndviReadings.length - 1].value : 0;

  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-xl font-bold text-foreground">Oracle</h1>
        <p className="text-sm text-muted-foreground mt-1">Live IoT telemetry, satellite data &amp; on-chain trigger events for {spv.spv_code}</p>
      </div>

      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <SummaryKPI icon={Wifi} label="Active Sensors" value={`${sensorReadings.length}/${sensorReadings.length}`} subtext="All online" />
        <SummaryKPI icon={Satellite} label="NDVI (Canopy)" value={ndviCurrent.toFixed(2)} subtext="Threshold: ≥ 0.55" />
        ...
        <TabsContent value="events" className="space-y-4">
          <Card>
            <CardHeader className="pb-2">
              ...
              <CardDescription className="text-xs">On-chain oracle events including payments, milestone verifications, sensor alerts, and disbursements.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-3">
              {oracleEvents.map((event) => <OracleEventCard key={event.id} event={event} />)}
            </CardContent>
          </Card>
        </TabsContent>
                ...
                <Card key={sensor.id}>
                  <CardHeader className="pb-2 space-y-1">
                    <div className="flex items-center justify-between">
                      ...
                      <Badge variant={statusBadgeVariant[sensor.status]} className="text-[10px]">
                        {sensor.status === "normal" ? <CircleCheck className="h-3 w-3 mr-1" /> : <AlertTriangle className="h-3 w-3 mr-1" />}
                        {sensor.status}
                      </Badge>
                    </div>
                  ...
                  <CardContent className="space-y-3">
                    <div className="flex items-baseline gap-1">
                      <span className={`text-2xl font-bold ${statusColor[sensor.status]}`}>{sensor.value}</span>
                      <span className="text-xs text-muted-foreground">{sensor.unit}</span>
                    </div>
                    {sensor.threshold_min != null && sensor.threshold_max != null && (
                      <SensorGauge current={sensor.value} min={sensor.threshold_min} max={sensor.threshold_max} unit={sensor.unit || ""} />
                    )}
                  </CardContent>
                </Card>
              );
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            <Card className="lg:col-span-2">
              <CardHeader>
                ...
                <CardDescription className="text-xs">Normalized Difference Vegetation Index. Oracle threshold: ≥ 0.55 for Milestone 3 disbursement.</CardDescription>
              </CardHeader>
              <CardContent>
                <ChartContainer config={ndviConfig} className="h-64 w-full aspect-auto">
                  <LineChart data={ndviReadings.map(r => ({ date: r.reading_date, value: r.value }))} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
                    <CartesianGrid strokeDasharray="3 3" className="stroke-border/30" />
                    <XAxis dataKey="date" tick={{ fontSize: 11 }} />
                    <YAxis domain={[0.3, 0.8]} tick={{ fontSize: 11 }} />
                    <ChartTooltip content={<ChartTooltipContent />} />
                    <Line type="monotone" dataKey="value" stroke="hsl(var(--primary))" strokeWidth={2} dot={{ r: 4, fill: "hsl(var(--primary))" }} />
                    <Line type="monotone" dataKey={() => 0.55} stroke="hsl(var(--destructive))" strokeDasharray="6 3" strokeWidth={1} dot={false} name="Threshold" />
                  </LineChart>
                </ChartContainer>
              </CardContent>
            </Card>
            <Card>
              <CardHeader><CardTitle className="text-sm">Canopy Health Summary</CardTitle></CardHeader>
              <CardContent className="space-y-4">
                <div className="space-y-2">
                  <div className="flex justify-between text-xs">
                    <span className="text-muted-foreground">Current NDVI</span>
                    <span className="font-bold text-primary">{ndviCurrent}</span>
                  </div>
                  <Progress value={(ndviCurrent / 0.8) * 100} className="h-2" />
                </div>
                <div className="space-y-2">
                  <div className="flex justify-between text-xs">
                  ...
                  <Badge variant="default" className="text-[10px]"><CircleCheck className="h-3 w-3 mr-1" />Threshold met — M3 disbursed</Badge>
                </div>
