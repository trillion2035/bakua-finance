...
import {
  Wallet, TrendingUp, DollarSign, Calendar, ArrowUpRight, ExternalLink, LogOut,
  Bell, ChevronRight, AlertTriangle, CheckCircle2, Activity, BarChart3
} from "lucide-react";
...
const mockAlerts = [
  {
    ...
    description: "Lagos Water Treatment SPV distributed +$2,130 to your wallet.",
    time: "2 hours ago",
    spvId: "INV-002",
  },
  {
    id: "A2",
    type: "success" as const,
    title: "Milestone verified — NDVI threshold met",
    description: "Kenya Solar Grid SPV: Sentinel-2 confirmed NDVI 0.63 exceeds 0.55. Milestone 3 disbursement authorized.",
    time: "1 day ago",
    spvId: "INV-001",
  },
  {
    ...
    title: "Sensor alert — soil moisture approaching lower bound",
    description: "Kenya Solar Grid SPV: Plot A reading 18.5% — approaching 18% minimum threshold.",
    time: "3 days ago",
    spvId: "INV-001",
  },
  {
    ...
    description: "Off-taker payment of 12,400,000 FCFA verified and logged on Base L2.",
    time: "5 days ago",
    spvId: "INV-001",
  },
  {
    id: "A5",
    type: "success" as const,
    title: "Quarterly compliance check passed",
    description: "All covenants within bounds. Next check: Jun 2025.",
    time: "6 days ago",
    spvId: "INV-001",
  },
];
      ...
      <div className="px-6 md:px-10 lg:px-16 py-10 max-w-[1200px] mx-auto">
        ...
        <div className="flex items-start justify-between mb-8">
          ...
          <div className="relative">
            <button
              ...
              className="p-2.5 rounded-xl bg-card border border-border hover:bg-secondary transition-all relative"
            >
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          <TabsList className="mb-6 bg-secondary/50 border border-border">
            <TabsTrigger value="portfolio" className="text-[13px] font-semibold gap-1.5 data-[state=active]:bg-card">
              <BarChart3 className="w-3.5 h-3.5" /> Portfolio
            </TabsTrigger>
            <TabsTrigger value="activity" className="text-[13px] font-semibold gap-1.5 data-[state=active]:bg-card">
            ...
            <div className="mb-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {mockInvestments.map((inv) => (
                  <button
                    key={inv.id}
                    onClick={() => navigate(`/investor/spv/${inv.id}`)}
                    className="bg-card rounded-xl p-5 border border-border text-left hover:border-primary/40 hover:shadow-[0_0_20px_-8px_hsl(var(--primary)/0.2)] transition-all group"
                  >
                    <div className="flex items-start justify-between mb-3">
                      <div>
                        <div className="font-display text-[15px] font-bold group-hover:text-primary transition-colors">{inv.spvName}</div>
          ...
          <TabsContent value="activity">
            <div className="space-y-3">
              <h2 className="font-display text-lg font-bold mb-4">Alerts & Updates</h2>
              {mockAlerts.map((alert) => (
                <button
                  key={alert.id}
                  onClick={() => navigate(`/investor/spv/${alert.spvId}`)}
                  className="w-full bg-card rounded-xl p-5 border border-border text-left hover:border-primary/30 transition-all group flex gap-4"
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      <span className="font-display text-[14px] font-bold group-hover:text-primary transition-colors">{alert.title}</span>
                    ...
                    <span className="text-[11px] text-muted-foreground/60 mt-1.5 block">{alert.time}</span>
                  </div>
                </button>
              ))}
            </div>
