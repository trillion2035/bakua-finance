...
// Sign facility document modal (reuses pattern from SignTermSheetModal)
...
export function AdminDeploymentPanel({ submission }: AdminDeploymentPanelProps) {
  ...
  // Check if all facility docs are signed (for legal close gating)
  ...
  // Check if incorporation cert is uploaded
  ...
  const handleViewDoc = async (doc: GeneratedDocument) => {
    if (doc.file_url && !doc.content) {
      // It's a file in storage, open it
  ...
  // Can mark incorporation complete only if cert uploaded
  const canCompleteIncorporation = (stage: DeploymentStage) => 
    stage.stage_key === "spv_incorporation" ? !!incorpCertUploaded : true;

  // Can mark legal close complete only if all facility docs signed
  const canCompleteLegalClose = (stage: DeploymentStage) => 
    stage.stage_key === "legal_close" ? allFacilityDocsSigned : true;

  const canCompleteStage = (stage: DeploymentStage) => 
    canCompleteIncorporation(stage) && canCompleteLegalClose(stage);

  const getCompletionBlocker = (stage: DeploymentStage): string | null => {
      ...
      <div className="border-t border-border pt-4 space-y-3">
        ...
        >
          <div className="flex items-center gap-2">
            ...
            {stages && stages.every(s => s.status === "completed") ? (
            ...
            {stages && stages.length > 0 && (
              <div className="space-y-2">
                ...
                {stages.map((stage) => {
                  ...
                  const blocker = stage.status === "in_progress" ? getCompletionBlocker(stage) : null;
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        {stage.status === "in_progress" && (
                          <Button
                            ...
                            disabled={completeStage.isPending || !canCompleteStage(stage)}
                        ...
                        <div className="space-y-1 pt-1">
                          {stageDocs.map((doc) => (
                            <div key={doc.id} className="flex items-center gap-2 text-xs bg-background rounded p-2">
                              ...
                              {/* Sign button for facility docs during legal close */}
