...
// Contract sources - each one standalone
const SOURCES: Record<string, string> = {
  InvestorPositionToken: `// SPDX-License-Identifier: MIT
...
contract InvestorPositionToken {
  ...
  OracleAdapter: `// SPDX-License-Identifier: MIT
pragma solidity 0.8.28;
contract OracleAdapter {
  ...
  SPVVault: `// SPDX-License-Identifier: MIT
pragma solidity 0.8.28;
...
contract SPVVault {
  ...
  DistributionWaterfall: `// SPDX-License-Identifier: MIT
pragma solidity 0.8.28;
...
serve(async (req) => {
  ...
  try {
    ...
    const { contract_name, network = "testnet", constructor_args = [] } = body;

    if (!contract_name || !SOURCES[contract_name]) {
      throw new Error(`Invalid contract_name. Must be one of: ${Object.keys(SOURCES).join(", ")}`);
    }
    ...
    console.log(`Compiled ${cName}: ABI ${abi.length} entries, bytecode ${bytecode.length} chars`);
    ...
    if (balance === 0n) throw new Error(`No ETH on ${networkName}`);

    const nonce = await provider.getTransactionCount(deployerAddress, "pending");
    const factory = new ethers.ContractFactory(abi, bytecode, wallet);
    const deployTx = await factory.deploy(...constructor_args, { nonce });
    ...
    return new Response(JSON.stringify({
      ...
      tx_hash: txHash,
      network: networkName,
      network_label: isMainnet ? "mainnet" : "testnet",
      explorer_url: `${explorerBase}/address/${address}`,
      abi,
